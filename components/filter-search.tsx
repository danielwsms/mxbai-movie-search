...
export function FilterSearch() {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      console.log(
        `Key: ${event.key}, Ctrl: ${event.ctrlKey}, Meta: ${event.metaKey}`
      );

      if (event.key === "2" && !isInTextField) {
    ...
    <Card className="p-0 shadow-none flex-1">
      <Form action="/" className="flex gap-2 relative">
        <Input
          ref={inputRef}
          name="filter"
          placeholder="Filter for anything you can think of"
          className="flex-1 pr-24"
          aria-label="Filter for anything you can think of, press 2 to focus"
        />
